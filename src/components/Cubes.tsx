...
const Cubes: React.FC<CubesProps> = ({
  ...
  cellGap,
  borderStyle = '1px solid rgba(81, 53, 101, 0.3)',
  faceColor = '#FEF6EB',
  shadow = false,
  autoAnimate = true,
  rippleOnClick = true,
  rippleColor = 'rgba(81, 53, 101, 0.15)',
  rippleSpeed = 2
}) => {
    ...
    (rowCenter: number, colCenter: number) => {
      ...
      sceneRef.current.querySelectorAll<HTMLDivElement>('.cube').forEach(cube => {
        ...
        if (dist <= radius) {
          ...
          const angleY = dx * pct * maxAngle;
          const angleX = -dy * pct * maxAngle;
          
          cube.style.transform = `rotateX(${angleX}deg) rotateY(${angleY}deg)`;
          cube.style.transition = `transform ${enterDur}s cubic-bezier(0.22, 1, 0.36, 1)`;
        } else {
          cube.style.transform = 'rotateX(0deg) rotateY(0deg)';
          cube.style.transition = `transform ${leaveDur}s cubic-bezier(0.22, 1, 0.36, 1)`;
        }
      });
    },
      ...
      Object.keys(rings)
        ...
        .forEach(ring => {
          ...
          cubes.forEach(cube => {
            ...
            faces.forEach(face => {
              face.style.transition = `background-color ${animDuration}s cubic-bezier(0.22, 1, 0.36, 1)`;
              face.style.transitionDelay = `${delay}s`;
              face.style.backgroundColor = rippleColor;
              
              setTimeout(() => {
                face.style.transitionDelay = '0s';
  ...
  return (
    <div className="fixed inset-0 w-full h-full" style={{ backgroundColor: faceColor }}>
